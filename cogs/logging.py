...
class Logging(commands.Cog):
    ...
    @commands.command(name="set-pattern")
    @commands.is_owner()
    async def set_pattern(self, ctx: Context, *, pattern: str):
        if await confirm(self.bot, ctx, "Set pattern?"):
            self.bot.pattern = fr"{pattern}"
    ...
    async def check_logs(self):
        ...
        try:
            with open(self.filename, "r") as f:
                data_raw = f.read()
                pattern = re.compile(self.bot.pattern)
